    ...
    <table class="cssTable1">
        ...
        <tr>
            ...
            <td class="cssTableInput">
                ...
                <br/>
                時間:
                <input type="text" id="email_time" readonly onclick="WdatePicker(WdatePickerOpt2);" style="width:173px;" value="<?=$print_date[chr_time]?>"/>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="cssTableInput" valign="top" colspan="2">
                ...
                <div style="height:369px; overflow:scroll; overflow-y:scroll; overflow-x:hidden;">
                    ...
                    <div id="item_<?=$value[int_id]?>" class="item gp_<?=$value[int_group]?>"><?=$value[chr_name]?><input type="hidden" id="code_<?=$value[chr_no]?>" value="<?=$value[chr_no]?>" class="no"></div>
                    <?php } ?>
                </div>
            </td>
        </tr>
...
<script>
    ...
    function reportInfo(){
        ...
        if(select_shop.length == all_shop.length){
            return { "item":no, "all_shop":1, "name": name, "num_of_day": num_of_day, "hide":hide, "mainItem":mainItem, "sort":sort, "print_weekday":print_weekday, "print_time":print_time}
        }else{
            var temp = { "item":no, "all_shop":0, "name": name, "num_of_day": num_of_day, "hide":hide, "mainItem":mainItem, "sort":sort, "print_weekday":print_weekday, "print_time":print_time}
